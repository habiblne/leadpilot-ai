...
function showResult(data) {
  ...
  text('#result-temperature', (q.temperature || '').toUpperCase());
...
form.addEventListener('submit', async (event) => {
  ...
  submitButton.textContent = 'Qualifying…';
  ...
  try {
    ...
    setStatus('success', data?.message || 'Lead qualified successfully.');
    showResult(data);
  } catch (error) {
    setStatus('error', error instanceof Error ? error.message : 'Something went wrong.');
  } finally {
    submitButton.disabled = false;
    submitButton.textContent = 'Qualify lead with AI';
  }
});
